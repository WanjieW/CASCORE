...
# CASCORE = function(Adj, Covariate, K, alpha = NULL, itermax = 100, startn = 10){
#   # Inputs:
#   # 1) Adj: an n by n symmetric adjacency matrix whose diagonals = 0 and positive entries = 1.
#   # 2) Covariate: an n by p covariate matrix
#   # 3) K: a positive integer which is no larger than n. This is the predefined number of communities.
...
#     d.net = eig(Adj); 
#     alphaupper = d.net[1]
#     alphalower = min(0.05, alphaupper/10); 
#     
...
#         vecn = vec[,2:K]/vec[,1];
#         result = kmeans(vecn, K, iter.max = itermax, nstart = startn); 
#         prop1[i] = result$tot.withinss;
#         est1[i,] = as.factor(result$cluster);
#         #        print(cluster(table(est1[i,], l))$error)
#       }
#       vecn = vec/apply(vec, 1, Norm);
#       result = kmeans(vecn, K, iter.max = itermax, nstart = startn); 
#       prop2[i] = result$tot.withinss;
#       est2[i,] = as.factor(result$cluster);
# #      print(cluster(table(est2[i,], label))$error)
#     }
#     #    print(prop1)
#     #    print(prop2)
#     if(sum(prop1 == 0) == 0){
#       est = est1[which.min(prop1), ]; 
#     }
#     else{
#       est = est2[which.min(prop2), ]; 
#     }
...
CASCOREv2  = function(Adj, Covariate, K, alpha = NULL, alphan = 5, itermax = 100, startn = 10){
  # Inputs:
  # 1) Adj: an n by n symmetric adjacency matrix whose diagonals = 0 and positive entries = 1.
  # 2) Covariate: an n by p covariate matrix
  # 3) K: a positive integer which is no larger than n. This is the predefined number of communities.
  ...
  if(is.null(alpha)){
    ...
    for(i in 1:alphan){
      ...
      vec = c$vectors
      vecn = vec[,2:K]/vec[,1];
      vecn[vecn > sqrt(log(n))] = sqrt(log(n));
      vecn[vecn < -1*sqrt(log(n))] = -1*sqrt(log(n));
      result = kmeans(vecn, K, iter.max = itermax, nstart = startn);
      if (result$ifault==4) { result = kmeans(X, K,  iter.max = itermax, nstart = startn, algorithm="Lloyd"); }
      prop2[i] = result$tot.withinss;
      est2[i,] = as.factor(result$cluster);
...
SCOREl2 = function(Adj, K, itermax = NULL, startn = NULL){
  # Inputs:
  # 1) Adj: an n by n symmetric adjacency matrix whose diagonals = 0 and positive entries = 1.
  # 2) K: a positive integer which is no larger than n. This is the predefined number of communities.
  ...
  g.eigen = eigen(Adj)
  V = g.eigen$vectors[,1:K];
  norml2 = apply(V, 1, function(x){sum(x^2)})
  if(sum(norml2==0) > 0) stop("Error! Zeroes in the first column")
  R = V / sqrt(norml2)
...
#jcdc = function(Adj, phi, K, itermax = 100, startn = 10){
# Inputs
# 1) Adj: adjacency matrix, n\times n
# 2) K: number of communities
# 3) phi: similarity matrices for individual features, n\times n\times p
#         For example: if features are f_1,\ldots,f_p, 
#         then phi_{ijk} is the similarity between f_k's for nodes i and j

## Initialize with spectral clustering
#D.inv = diag(1./(sqrt(apply(Adj, 1, sum))+1e-7));
#Laplacian = D.inv %*% Adj %*% D.inv;
#L.svd = svd(Laplacian);
#U.K = L.svd$u[, 1:K];
#spec.cluster = kmeans(U.K, K, iter.max = itermax, nstart = startn)$cluster;
#n = dim(Adj)[1];# check with Yaofang
#G.fit = array(0, c(n, K));
#for(k in 1:K){
...
# JCDC(A, SimilarityCube, DimensionOfFeatures, InitialCommunityGuess, StepSize, K, NumberOfIterations,
#	NumberOfMaxGradientRounds, NumberOfMaxTabuRounds, W_max, Lambda, RandomStart(0/1), Alpha)
#w_max = 1; #check with Yaofang
#result <- JCDC(A, phi, p, G.fit, 1, K, 20, 30, 20, 1, 2, 0, 1); #I change w_max to be 1, since w_max cannot be recognized
#Error: requested size is too large
#est = as.vector(result$G.fit %*% (1:K));
#return(est)
#}
