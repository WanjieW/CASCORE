...
K1 = 3; K2 = K1 + 2; prob1 = 0.9; #there are 10% nodes mis-matched
aseq = seq(0.1, 0.9, by = 0.1);
mu = 0.3;
errorall = matrix(0, nrow = length(aseq), ncol = 10) #store the error rate in each repetition
colnames(errorall) = c("New", "CASC", "ADMM", "Net_based", "Cov_based", "New,small", 
...
K1 = 3; K2 = K1 + 2; prob1 = 0.9; #there are 10% nodes mis-matched
a = 0.5; museq = seq(0.1, 0.5, by = 0.02);
errorall = matrix(0, nrow = length(museq), ncol = 10) #store the error rate in each repetition
colnames(errorall) = c("New", "CASC", "ADMM", "Net_based", "Cov_based", "New,small", 
                       "CASC,small", "ADMM,small", "Cov_based, small", "giant/all")
errsd = errorall;
...
K1 = 3; K2 = K1 + 2; 
a = 0.5; mu = 0.3;
#prob1seq = c(0.5, 0.7, 1);
prob1seq = seq(0.3, 1, by = 0.1);
errorall = matrix(0, nrow = length(prob1seq), ncol = 10) #store the error rate in each repetition
colnames(errorall) = c("New", "CASC", "ADMM", "Net_based", "Cov_based", "New,small", 
                       "CASC,small", "ADMM,small", "Cov_based, small", "giant/all")
errsd = errorall;
