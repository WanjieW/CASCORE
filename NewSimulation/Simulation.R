...
############ End Parameter Set  ############
errormat = matrix(0, nrow = repetition, ncol = 10) #store the error rate in each repitition
colnames(errormat) = c("New", "CASC", "ADMM", "Net_based", "Cov_based", "New,small", 
...
#    mu = 0.2;
  ...
  for(smalli in 1:K2){
    Qsmall[(smalli-1)*(smallp/K2)+(1:(smallp/K2)), smalli] = mu + 0.2;
  }
#    signal_big = rbinom(bigp, 1, 2*sqrt(bigp)/bigp);
  ...
  est_cascore = CASCORE(A, Dbig, K1, alpha = alpha)
  ...
  # New approach: CA-SCORE
  degree = colSums(A); alpha = mean(degree)/2;
  est_cascore = CASCORE(A, Dsmall, K1, alpha = alpha)
  comp = table(est_cascore, l);
  err_cascore = cluster(comp)$error;
  errormat[ii, "New,small"] = err_cascore;
